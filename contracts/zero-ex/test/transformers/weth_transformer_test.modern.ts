...
describe('WethTransformer - Modern Tests', function () {
    ...
    function encodeWethTransformerData(params: { amount: bigint; token: string }): string {
        // Simplified encoding for test purposes
        const abiCoder = ethers.AbiCoder.defaultAbiCoder();
        return abiCoder.encode(['uint256', 'address'], [params.amount, params.token]);
    }
    ...
    it('can unwrap some WETH', async function () {
        ...
        console.log(
            `✅ Unwrapped ${ethers.formatEther(unwrapAmount)} WETH, ${ethers.formatEther(amount - unwrapAmount)} WETH remaining`,
        );
    });
    ...
    it('can wrap some ETH', async function () {
        ...
        console.log(
            `✅ Wrapped ${ethers.formatEther(wrapAmount)} ETH, ${ethers.formatEther(amount - wrapAmount)} ETH remaining`,
        );
    });
});
