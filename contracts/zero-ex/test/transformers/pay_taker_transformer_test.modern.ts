...
describe('PayTakerTransformer - Modern Tests', function () {
    ...
    function encodePayTakerTransformerData(params: { amounts: bigint[]; tokens: string[] }): string {
        // Simplified encoding for test purposes
        // In real implementation, this would use the actual encoding logic
        const abiCoder = ethers.AbiCoder.defaultAbiCoder();
        return abiCoder.encode(['uint256[]', 'address[]'], [params.amounts, params.tokens]);
    }

    it('can transfer a token and ETH', async function () {
        const taker = generateRandomAddress();
        const amounts = [getRandomInteger('1', '10'), getRandomInteger('1', '10')];
        ...
        console.log(
            `✅ Transferred ${ethers.formatEther(amounts[0])} tokens and ${ethers.formatEther(amounts[1])} ETH`,
        );
    });
    ...
    it('can transfer less than the balance of a token and ETH', async function () {
        const taker = generateRandomAddress();
        const amounts = [getRandomInteger('2', '10'), getRandomInteger('2', '10')];
        ...
        console.log(
            `✅ Transferred partial amounts: ${ethers.formatEther(transferAmounts[0])} tokens, ${ethers.formatEther(transferAmounts[1])} ETH`,
        );
    });
});
