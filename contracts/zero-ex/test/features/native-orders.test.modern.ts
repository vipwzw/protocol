...
describe('Native Orders Feature - Modern Tests', function () {
    ...
    async function deployContractsAsync(): Promise<void> {
        ...
        testToken = await TestTokenFactory.deploy('Test Token', 'TEST', 18, ethers.parseEther('1000000'));
    ...
    describe('Order Creation', function () {
        it('should create valid limit orders', async function () {
            ...
            console.log(
                `✅ Created limit order: ${ethers.formatEther(order.makerAmount.toString())} TEST for ${ethers.formatEther(order.takerAmount.toString())} ETH`,
            );
        });
        ...
        it('should create valid RFQ orders', async function () {
            ...
            console.log(
                `✅ Created RFQ order: ${ethers.formatEther(rfqOrder.makerAmount.toString())} TEST for ${ethers.formatEther(rfqOrder.takerAmount.toString())} ETH`,
            );
        });
    ...
    describe('Order Validation', function () {
        it('should validate order expiry times', async function () {
            ...
            console.log(
                `✅ Validated expiry times: valid=${validOrder.expiry}, expired=${expiredOrder.expiry}, current=${currentTime}`,
            );
        });
        ...
        it('should validate order amounts', async function () {
            ...
            console.log(
                `✅ Validated amounts: maker=${ethers.formatEther(order.makerAmount.toString())}, taker=${ethers.formatEther(order.takerAmount.toString())}`,
            );
        });
    });
    ...
    describe('Order Status Management', function () {
        ...
        it('should track order fill status', async function () {
            ...
            console.log(
                `✅ Initial status: filled=${initialStatus.filledAmount}, cancelled=${initialStatus.cancelled}`,
            );
        });
        ...
        it('should handle partial fills', async function () {
            ...
            console.log(
                `✅ Partial fill: ${ethers.formatEther(partialFillAmount.toString())} of ${ethers.formatEther(order.makerAmount.toString())} tokens`,
            );
        });
    });
    ...
    describe('Gas Optimization', function () {
        it('should estimate gas for order operations', async function () {
            ...
            console.log(
                `⛽ Gas estimates: create=${estimatedGas.createOrder}, fill=${estimatedGas.fillOrder}, cancel=${estimatedGas.cancelOrder}`,
            );
        });
    ...
    describe('Error Handling', function () {
        it('should handle insufficient balances gracefully', async function () {
            ...
            console.log(
                `❌ Testing with impossible amount: ${ethers.formatEther(order.makerAmount.toString())} tokens`,
            );
        });
