...
describe('MultiplexFeature - Complete Modern Tests', function () {
    ...
    async function deployContractsAsync(): Promise<void> {
        ...
        transformFeature = new ethers.Contract(verifyingContract, transformERC20.interface, ethers.provider);
    ...
    async function migrateMultiplexFeatureAsync(): Promise<void> {
        ...
        multiplex = new ethers.Contract(await zeroEx.getAddress(), featureImpl.interface, ethers.provider);
    ...
    async function createUniswapV2PoolAsync(
        ...
        try {
            ...
            if (error.message && error.message.includes('POOL_ALREADY_EXISTS')) {
                console.log(
                    `ℹ️ Pool already exists for ${await token0.getAddress()} / ${await token1.getAddress()}, using existing pool`,
                );
    ...
    async function createUniswapV3PoolAsync(
        ...
        const result = await uniV3Factory.createPool(await token0.getAddress(), await token1.getAddress(), POOL_FEE);
        const receipt = await result.wait();
    ...
    async function getRfqSubcallAsync(
        ...
        const makerToken =
            rfqOrder.makerToken === (weth.target || weth.target)
                ? weth
                : rfqOrder.makerToken === (zrx.target || zrx.target)
                  ? zrx
                  : dai;
        ...
        const data = ethers.AbiCoder.defaultAbiCoder().encode(
            [
                'tuple(address maker, address taker, address makerToken, address takerToken, uint256 makerAmount, uint256 takerAmount, address txOrigin, bytes32 pool, uint64 expiry, uint256 salt)',
                'bytes',
            ],
            [rfqOrder, signature],
        );
    ...
    async function getOtcSubcallAsync(
        ...
        const makerToken =
            otcOrder.makerToken === (weth.target || weth.target)
                ? weth
                : otcOrder.makerToken === (zrx.target || zrx.target)
                  ? zrx
                  : dai;
        ...
        const data = ethers.AbiCoder.defaultAbiCoder().encode(
            [
                'tuple(address maker, address taker, address makerToken, address takerToken, uint256 makerAmount, uint256 takerAmount, address txOrigin, uint256 nonceBucket, uint256 nonce, uint64 expiry)',
                'bytes',
            ],
            [otcOrder, signature],
        );
    ...
    function getUniswapV2MultiHopSubcall(tokens: string[], isSushi = false): MultiHopSellSubcall {
        const data = ethers.AbiCoder.defaultAbiCoder().encode(['address[]', 'bool'], [tokens, isSushi]);
    ...
    function getUniswapV3MultiHopSubcall(tokens: any[]): MultiHopSellSubcall {
        ...
        const data = ethers.AbiCoder.defaultAbiCoder().encode(['address[]', 'uint24[]'], [tokenAddresses, fees]);
    ...
    describe('batch sells', function () {
        describe('multiplexBatchSellTokenForToken', function () {
            ...
            it('LiquidityProvider, UniV3, Sushiswap', async function () {
                ...
                const sellAmount =
                    liquidityProviderSubcall.sellAmount + uniV3Subcall.sellAmount + sushiswapSubcall.sellAmount - 1n;
        ...
        describe('multiplexBatchSellEthForToken', function () {
            it('can sell ETH for tokens via RFQ', async function () {
                ...
                const result = await multiplex
                    .connect(taker)
                    .multiplexBatchSellEthForToken(dai.target || dai.target, [rfqSubcall], ZERO_AMOUNT, {
            ...
            it('can sell ETH for tokens via UniswapV2', async function () {
                ...
                const result = await multiplex
                    .connect(taker)
                    .multiplexBatchSellEthForToken(dai.target || dai.target, [uniswapSubcall], ZERO_AMOUNT, {
    ...
    describe('multihop sells', function () {
        ...
        describe('multiplexMultiHopSellEthForToken', function () {
            it('can execute ETH to token multi-hop', async function () {
                ...
                await createUniswapV2PoolAsync(uniV2Factory, weth, dai);
                const uniswapSubcall = getUniswapV2MultiHopSubcall([await weth.getAddress(), await dai.getAddress()]);
        ...
        describe('multiplexMultiHopSellTokenForEth', function () {
            it('can execute token to ETH multi-hop', async function () {
                ...
                const uniswapSubcall = getUniswapV2MultiHopSubcall([await zrx.getAddress(), await weth.getAddress()]);
