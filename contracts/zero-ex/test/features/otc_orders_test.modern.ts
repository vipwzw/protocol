...
describe('OtcOrdersFeature - Modern Tests', function () {
    ...
    async function deployContractsAsync(): Promise<void> {
        ...
        await approveTokensForAccounts([makerToken, takerToken, wethToken], accounts, deployment.verifyingContract);
    ...
    async function setupTestUtilsAsync(): Promise<void> {
        ...
        testUtils = {
            ...
            // Fill OTC order
            fillOtcOrderAsync: async function (
                order: any,
                fillAmount?: bigint,
                txOriginAddr?: any,
            ...
            fillOtcOrderWithEthAsync: async function (order: any, fillAmount?: bigint) {
                ...
                return await deployment.zeroEx
                    .connect(taker)
                    .fillOtcOrderWithEth(order, signature, amount, { value: amount });
            },

            // Fill taker signed OTC order
            fillTakerSignedOtcOrderAsync: async function (
                order: any,
                txOriginAddr?: any,
                takerAddr?: any,
                unwrapWeth: boolean = false,
            ...
            createOtcOrderFilledEventArgs: function (order: any, fillAmount?: bigint) {
                ...
                const { makerTokenFilledAmount, takerTokenFilledAmount } = this.computeOtcOrderFilledAmounts(
                    order,
                    filledAmount,
                );
    ...
    async function assertExpectedFinalBalancesFromOtcOrderFillAsync(order: any, fillAmount?: bigint): Promise<void> {
        const filledAmount = fillAmount || order.takerAmount;
        const { makerTokenFilledAmount, takerTokenFilledAmount } = testUtils.computeOtcOrderFilledAmounts(
            order,
            filledAmount,
        );
    ...
    describe('fillOtcOrder()', function () {
        ...
        it('can partially fill an order', async function () {
            ...
            console.log(
                `✅ Partially filled OTC order: ${ethers.formatEther(fillAmount.toString())} / ${ethers.formatEther(order.takerAmount.toString())}`,
            );
        });
        ...
        it('cannot fill an order with wrong tx.origin', async function () {
            ...
            await expect(testUtils.fillOtcOrderAsync(order, order.takerAmount, notTaker)).to.be.revertedWith(
        ...
        it('cannot fill an order with wrong taker', async function () {
            ...
            await expect(testUtils.fillOtcOrderAsync(order)).to.be.revertedWith('OrderNotFillableByTakerError');
        ...
        it('cannot fill an order with a zero tx.origin', async function () {
            ...
            await expect(testUtils.fillOtcOrderAsync(order)).to.be.revertedWith('OrderNotFillableByOriginError');
        ...
        it('cannot fill an expired order', async function () {
            ...
            await expect(testUtils.fillOtcOrderAsync(order)).to.be.revertedWith('OrderNotFillableError');
        ...
        it('cannot fill order with bad signature', async function () {
            ...
            await expect(testUtils.fillOtcOrderAsync(badOrder)).to.be.revertedWith('OrderNotSignedByMakerError');
        ...
        it('cannot fill two orders with the same nonceBucket and nonce', async function () {
            ...
            await expect(testUtils.fillOtcOrderAsync(order2)).to.be.revertedWith('OrderNotFillableError');
        ...
        it('can fill a WETH buy order and receive ETH', async function () {
            ...
            console.log(
                `✅ Filled WETH buy order and received ETH: ${ethers.formatEther(order.makerAmount.toString())}`,
            );
        });
        ...
        it('reverts if `unwrapWeth` is true but maker token is not WETH', async function () {
            ...
            await expect(testUtils.fillOtcOrderAsync(order, order.takerAmount, taker, true)).to.be.revertedWith(
                'MAKER_TOKEN_NOT_WETH',
            );
        ...
        it('allows for fills on orders signed by a approved signer', async function () {
            ...
            await contractWallet
                .connect(contractWalletOwner)
                .registerAllowedOrderSigner(contractWalletSigner.target, true);
        ...
        it('disallows fills if the signer is revoked', async function () {
            ...
            await contractWallet
                .connect(contractWalletOwner)
                .registerAllowedOrderSigner(contractWalletSigner.target, false);

            // Fill should revert
            await expect(otcFeature.connect(taker).fillOtcOrder(order, sig, order.takerAmount)).to.be.revertedWith(
    ...
    describe('fillOtcOrderWithEth()', function () {
        ...
        it('Can fill an order with ETH (takerToken=ETH)', async function () {
            ...
            expect(Number(makerEthBalanceAfter - makerEthBalanceBefore, 'maker balance')).to.equal(
                Number(order.takerAmount),
            );
        ...
        it('Can refund excess ETH is msg.value > order.takerAmount (takerToken=WETH)', async function () {
            ...
            console.log(
                `✅ Refunded excess ETH (WETH): sent ${ethers.formatEther(fillAmount.toString())}, used ${ethers.formatEther(order.takerAmount.toString())}`,
            );
        });
        ...
        it('Can refund excess ETH is msg.value > order.takerAmount (takerToken=ETH)', async function () {
            ...
            const takerEthBalanceAfter = await ethers.provider.getBalance(taker.target);
            expect(Number(takerEthBalanceBefore - takerEthBalanceAfter, 'taker eth balance')).to.equal(
                Number(order.takerAmount),
            );
            ...
            console.log(
                `✅ Refunded excess ETH (ETH): sent ${ethers.formatEther(fillAmount.toString())}, used ${ethers.formatEther(order.takerAmount.toString())}`,
            );
        });
        ...
        it('Cannot fill an order if taker token is not ETH or WETH', async function () {
            ...
            await expect(testUtils.fillOtcOrderWithEthAsync(order)).to.be.revertedWith('INVALID_TAKER_TOKEN');
    ...
    describe('fillTakerSignedOtcOrder()', function () {
        ...
        it('cannot fill an order with wrong tx.origin', async function () {
            ...
            await expect(testUtils.fillTakerSignedOtcOrderAsync(order, notTxOrigin)).to.be.revertedWith(
                'OrderNotFillableByOriginError',
            );
        ...
        it('can fill a WETH buy order and receive ETH', async function () {
            ...
            console.log(
                `✅ Filled taker signed WETH order and received ETH: ${ethers.formatEther(order.makerAmount.toString())}`,
            );
        });
        ...
        it('reverts if `unwrapWeth` is true but maker token is not WETH', async function () {
            ...
            await expect(testUtils.fillTakerSignedOtcOrderAsync(order, txOrigin, taker, true)).to.be.revertedWith(
                'MAKER_TOKEN_NOT_WETH',
            );
