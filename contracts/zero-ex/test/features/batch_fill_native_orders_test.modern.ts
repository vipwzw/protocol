...
describe('BatchFillNativeOrders Feature - Modern Tests (Fixed)', function () {
    ...
    async function deployContractsAsync(): Promise<void> {
        ...
        // 4. 批量授权代币
        await approveTokensForAccounts([makerToken, takerToken], [maker, taker], deployment.verifyingContract);
        ...
        testUtils = {
            ...
            createTestLimitOrder: function (fields: any = {}) {
                return new LimitOrder({
                    ...
                    takerTokenFeeAmount: new BigNumber(
                        fields.takerTokenFeeAmount?.toString() || ZERO_AMOUNT.toString(),
                    ),
                    ...
                    salt: new BigNumber(
                        fields.salt?.toString() || Math.floor(Math.random() * 1000000000000).toString(),
                    ),
            ...
            createLimitOrderFilledEventArgs: function (order: any, fillAmount?: bigint) {
                return {
                    ...
                    makerTokenFilledAmount: fillAmount
                        ? (fillAmount * order.makerAmount) / order.takerAmount
                        : order.makerAmount,
    ...
    describe('batchFillLimitOrders', function () {
        ...
        it('fills multiple orders and pays protocol fees in ETH', async function () {
            ...
            console.log(
                `📊 BatchFillNativeOrders Result: Expected ${orders.length} fills, Found ${fillEvents.length} events`,
            );

            if (fillEvents.length > 0) {
                console.log(
                    `🎉 SUCCESS: BatchFillNativeOrders is working! Found ${fillEvents.length} LimitOrderFilled events`,
                );
    ...
    describe('batchFillRfqOrders', function () {
        ...
        it('partially fills multiple orders', async function () {
            ...
            const tx = await batchFillFeature.connect(taker).batchFillRfqOrders(orders, signatures, fillAmounts, false);
            ...
            fillAmounts.forEach((amount, i) => {
                console.log(
                    `   Order ${i}: ${ethers.formatEther(amount.toString())} / ${ethers.formatEther(orders[i].takerAmount.toString())}`,
                );
            });
        ...
        it('reverts on unfillable order when revertIfIncomplete=true', async function () {
            const fillableOrders = [testUtils.createTestRfqOrder(), testUtils.createTestRfqOrder()];
        ...
        it('reverts on incomplete fill when revertIfIncomplete=true', async function () {
            ...
            console.log(
                `✅ Correctly reverted on incomplete fill: ${ethers.formatEther(remainingAmount)} / ${ethers.formatEther(partiallyFilledOrder.takerAmount)}`,
            );
        });
    });
});
