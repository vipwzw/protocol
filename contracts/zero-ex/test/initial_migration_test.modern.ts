    ...
    async function performInitialMigrationAsync(): Promise<void> {
        ...
        try {
            ...
            if (migrator.initializeZeroEx) {
                await migrator.connect(admin).initializeZeroEx(owner.target, await zeroEx.getAddress(), {
    ...
    describe('🗳️ Ownership and Access Control', function () {
        ...
        it('should enforce migration permissions', async function () {
            ...
            try {
                ...
                if (migrator.initializeZeroEx) {
                    await expect(
                        migrator.connect(user2).initializeZeroEx(unauthorizedUser, await zeroEx.getAddress(), {}),
                    ).to.be.reverted;
