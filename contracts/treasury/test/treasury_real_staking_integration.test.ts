...
describe('Treasury Governance with Real Staking Integration', function() {
    ...
    async function setupBalancesAsync(): Promise<void> {
        ...
        await ((zrx as any).connect(admin) as any).approve(await (stakingContract as any).getAddress(), ethers.parseEther('10000'));
        await ((zrx as any).connect(poolOperator) as any).approve(await (stakingContract as any).getAddress(), ethers.parseEther('10000'));
        await ((zrx as any).connect(delegator) as any).approve(await (stakingContract as any).getAddress(), ethers.parseEther('10000'));
        await ((zrx as any).connect(relayer) as any).approve(await (stakingContract as any).getAddress(), ethers.parseEther('10000'));
    ...
    async function setupStakingPoolsAsync(): Promise<void> {
        ...
        const receipt = await tx.wait(); if (!receipt) throw new Error("Transaction receipt is null");
    ...
    describe('⚡ Real Staking Operations', function() {
        ...
        it('should allow delegation to real staking pools', async function() {
            ...
            const delegatedAfter = await (stakingContract as any).getStakeDelegatedToPoolByOwner(relayer.address, defaultPoolId);
    ...
    describe('🗳️ Real Governance Integration', function() {
        ...
        it('should create proposals with real staking power validation', async function() {
            ...
            const tx = await (treasury as any).connect(delegator).propose(
                proposalActions,
                executionEpoch,
