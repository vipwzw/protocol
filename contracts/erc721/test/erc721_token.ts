import { constants, verifyERC721TransferEvent } from '@0x/utils';
...
describe('ERC721Token', () => {
    ...
    before(async () => {
        ...
        token = await tokenFactory.deploy('DummyNFT', 'DNFT');
    ...
    describe('transferFrom', () => {
        ...
        it('should revert if spender does not own the token, is not approved, and is not approved for all', async () => {
            ...
            await expect(token.connect(signer).transferFrom(from, to, tokenId)).to.be.revertedWith(
    ...
    describe('safeTransferFrom without data', () => {
        ...
        it('should revert if transferring to a contract address without onERC721Received', async () => {
            ...
            const contract = await contractFactory.deploy('DummyNFT', 'DNFT');
            const from = owner;
    ...
    describe('safeTransferFrom with data', () => {
        ...
        it('should revert if transferring to a contract address without onERC721Received', async () => {
            ...
            const from = owner;
            const to = await contract.getAddress();
            await expect(token['safeTransferFrom(address,address,uint256,bytes)'](from, to, tokenId, data)).to.be
                .reverted;
        });
        it('should revert if onERC721Received does not return the correct value', async () => {
            const signers = await ethers.getSigners();
            ...
            const to = await invalidErc721Receiver.getAddress();
            await expect(
                token['safeTransferFrom(address,address,uint256,bytes)'](from, to, tokenId, data),
            ).to.be.revertedWith(RevertReason.Erc721InvalidSelector);
        });
        it('should transfer to contract and call onERC721Received with correct return value', async () => {
            const from = owner;
            const to = await erc721Receiver.getAddress();
