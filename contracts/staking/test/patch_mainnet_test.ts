...
describe('Staking patch mainnet fork tests', () => {
    ...
    before(async () => {
        ...
        stakingProxyContract = new StakingProxyContract(STAKING_PROXY, signer, undefined, abis);
    ...
    it('Staking proxy successfully attaches to patched logic', async () => {
        const tx = await stakingProxyContract.attachStakingContract(patchedStakingPatchContract.address);
        const receipt = await tx.wait();
        const events = filterLogsToArguments(receipt.logs, 'StakingContractAttachedToProxy');
        expect(events).to.have.lengthOf(1);
        expect(events[0].newStakingContractAddress).to.equal(patchedStakingPatchContract.address);
        logUtils.log(`${receipt.gasUsed?.toString()} gas used`);
    });

    it('Patched staking handles 0 gas protocol fees', async () => {
        const [signer] = await ethers.getSigners();
        const staking = new StakingPatchContract(STAKING_PROXY, signer, undefined, abis);
        ...
        expect(filterLogsToArguments(zeroFeeReceipt.logs, 'StakingPoolEarnedRewardsInEpoch')).to.be.empty;
        ...
        expect(filterLogsToArguments(nonZeroFeeReceipt.logs, 'StakingPoolEarnedRewardsInEpoch')).to.have.lengthOf(1);
    });
});
