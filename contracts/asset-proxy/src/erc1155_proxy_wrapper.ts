...
export class ERC1155ProxyWrapper {
     ...
     * @param authorizedSender sender of `transferFrom` transaction
     */
    public async transferFromRawAsync(txData: string, authorizedSender: string): Promise<any> {
        // 获取 signer
        const signers = await (hre as any).ethers.getSigners();
        const signer =
            signers.find((s: any) => s.address.toLowerCase() === authorizedSender.toLowerCase()) || signers[0];
    ...
    public async setBalancesAndAllowancesAsync(): Promise<ERC1155HoldingsByOwner> {
        ...
        for (const dummyContract of this._dummyTokenContracts) {
            ...
            for (const i of _.times(ERC1155_CONSTANTS.NUM_ERC1155_FUNGIBLE_TOKENS_MINT)) {
                ...
                const tokenId = await this._mintFungibleTokensAsync(dummyContract, this._tokenOwnerAddresses, amounts);
                ...
                for (const tokenOwnerAddress of this._tokenOwnerAddresses) {
                    try {
                        ...
                        const ownerSigner = signers.find(
                            (s: any) => s.address.toLowerCase() === tokenOwnerAddress.toLowerCase(),
                        );
                        if (ownerSigner) {
                            // 使用持有者的签名者连接合约并授权
            ...
            for (const j of _.times(ERC1155_CONSTANTS.NUM_ERC1155_NON_FUNGIBLE_TOKENS_MINT)) {
                const [tokenId, nftIds] = await this._mintNonFungibleTokensAsync(
                    dummyContract,
                ...
                for (let i = 0; i < this._tokenOwnerAddresses.length; i++) {
                    ...
                    nonFungibleHoldingsByOwner[tokenOwnerAddress][dummyAddress][tokenIdAsString].push(
                        BigInt(nftIds[i].toString()),
                    );
                }
            }
        }
    ...
    public async setProxyAllowanceForAllAsync(
        ...
        const signers = await (hre as any).ethers.getSigners();
        const userSigner =
            signers.find((s: any) => s.address.toLowerCase() === userAddress.toLowerCase()) || signers[0];
    ...
    private async _mintFungibleTokensAsync(
        ...
        const signers = await (hre as any).ethers.getSigners();
        const ownerSigner =
            signers.find((s: any) => s.address.toLowerCase() === this._contractOwnerAddress.toLowerCase()) ||
            signers[0];
        ...
        // Mint 代币给受益者
        await contractWithSigner.mintFungible(
            tokenId.toString(),
            beneficiaries,
            tokenAmounts.map(a => a.toString()),
        );
    ...
    private async _getBalancesAsync(contract: ERC1155Mintable, owners: string[], tokens: bigint[]): Promise<bigint[]> {
