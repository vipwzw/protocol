...
const fixPatterns = [
    ...
    {
        pattern:
            /(const\s+receipt\s*=\s*await\s+.*\.wait\(\);\s*)([\s\S]*?)(return\s*\{\s*[\s\S]*?logs:\s*receipt\.logs\s+as\s+any\s+as\s+DecodedLogs[,\s]*[\s\S]*?\})/g,
        replacement: `$1
            // 使用通用日志解析工具
...
function applyFixesToFile(filePath: string): void {
    ...
    if (!content.includes('bridge_event_helpers')) {
        ...
        if (artifactsImportMatch) {
            content = content.replace(artifactsImportMatch[0], UNIVERSAL_IMPORTS + '\n' + artifactsImportMatch[0]);
            modified = true;
            console.log(`  ✅ 添加了通用导入`);
        }
    }
    ...
    for (const file of bridgeTestFiles) {
        try {
            ...
            const { stdout } = await execPromise(
                `npx hardhat test ${file} --no-compile --grep "returns success bytes" --timeout 5000`,
            );
