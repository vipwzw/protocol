...
export abstract class RevertError extends Error {
    ...
    public static decode(bytes: string | Buffer | RevertError, coerce: boolean = false): RevertError {
        ...
        const _bytes = bytes instanceof Buffer ? ethUtil.bufferToHex(bytes) : ethUtil.addHexPrefix(bytes as string);
        ...
        try {
            ...
            throw new Error(
                `Bytes ${_bytes} cannot be decoded as a revert error of type ${instance.signature}: ${err instanceof Error ? err.message : String(err)}`,
            );
        }
    }
