...
function isAbiDataEqual(name: ParamName, type: string, x: any, y: any): boolean {
    ...
    if (_.startsWith(type, 'tuple(')) {
        ...
        if (types.length !== (name as any).names.length) {
            throw new Error(
                `Internal error: parameter types/names length mismatch (${types.length} != ${(name as any).names.length})`,
            );
        }
        for (let i = 0; i < types.length; i++) {
            ...
            //  }
            //
            const nestedName = _.isString((name as any).names[i])
                ? ((name as any).names[i] as string)
                : (((name as any).names[i] as NestedParamName).name as string);
            if (!isAbiDataEqual((name as any).names[i], types[i], x[nestedName], y[nestedName])) {
                return false;
            }
        }
        return true;
