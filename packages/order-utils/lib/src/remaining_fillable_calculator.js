...
class RemainingFillableCalculator {
    constructor(orderFee, orderAssetAmount, isPercentageFee, transferrableAssetAmount, transferrableFeeAmount, remainingOrderAssetAmount) {
        ...
        this._remainingOrderFeeAmount = orderAssetAmount === 0n
            ? 0n
            : (remainingOrderAssetAmount * orderFee) / orderAssetAmount;
    }
    ...
    _hasSufficientFundsForFeeAndTransferAmount() {
        if (this._isPercentageFee) {
            ...
            const hasSufficientFundsForTransferAmount = this._transferrableAssetAmount >= this._remainingOrderAssetAmount;
    ...
    _calculatePartiallyFillableAssetAmount() {
        ...
        const fillableTimesInFeeBaseUnits = this._transferrableFeeAmount < this._remainingOrderFeeAmount
        ...
        let partiallyFillableAssetAmount;
        if (this._isPercentageFee) {
            ...
            partiallyFillableAssetAmount = (totalAssetPooled * this._orderAssetAmount) / ((orderToFeeRatio + 1n) * this._orderFee);
        }
        else {
        ...
        const partiallyFillableAmount = partiallyFillableAssetAmount < partiallyFillableFeeAmount
